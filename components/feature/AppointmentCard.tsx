...
export const AppointmentCard = React.memo(function AppointmentCard({
  ...
  const { id, client_name, service, appointment_time, status, client_phone } = appointment;
  ...
  return (
    ...
    >
      <View style={styles.timeBlock}>
        <Text style={styles.time}>{appointment_time}</Text>
      ...
      <View style={styles.info}>
        <Text style={styles.name} numberOfLines={1}>{client_name}</Text>
        ...
        {client_phone ? (
          <Text style={styles.phone} numberOfLines={1}>{client_phone}</Text>
        ) : null}
      </View>
      ...
      {showActions && status === 'booked' ? (
        ...
        <View style={styles.actions}>
          <Pressable
            ...
            onPress={() => onStatusChange?.(id, 'in_progress')}
            hitSlop={4}
          >
...
const styles = StyleSheet.create({
  ...
  timeBlock: { alignItems: 'center', width: 44, gap: 4 },
  time: { fontSize: FontSize.sm, fontWeight: FontWeight.bold, color: Colors.primary },
  ...
  avatarText: { fontSize: FontSize.sm, fontWeight: FontWeight.bold, color: Colors.accentLight },
  ...
  startBtn: { backgroundColor: Colors.primaryMuted, borderColor: Colors.primary },
  doneBtn: { backgroundColor: Colors.successMuted, borderColor: Colors.success, width: 36, height: 36, borderRadius: 18 },
});
